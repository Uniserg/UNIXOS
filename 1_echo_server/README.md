...
**Ответ:** клиентский сокет делает connect к серверному; 
   серверный сокет привязывается к какому-то хосту и порту и запускается слушатель.
   
   
2. Как можно передавать через сокеты текстовую информацию?

**Ответ:** методом _send_ от клиента, и чтением кусками через метод _recv_

3. Какие операции с сокетами блокируют выполнение программы?

**Ответ:**
bind, accept - операции, которые должны вернуть результат(ответ).

4. Что такое неблокирующие сокеты?

**Ответ:** 

Выполнение операций с неблокирующим сокетом не приостанавливает работу приложения. О завершении работы функций приложение уведомляется. Данный режим реализован как альтернатива блокирующему режиму в потоке (т.е. при данном режиме поток попросту бесполезен)
...
**Ответ:** Системный вызов bind, listener и accept

7. На каком уровне модели OSI работают сокеты?

**Ответ:** На транспортном уровне.
